...
@Setter
@Getter
public class CCGUtil implements Callable<List<CCGRecord>> {

  private String fileName;

  @Autowired private ResourceLoader resourceLoader;

  public CCGUtil(String fileName) {
  ...
  public List<CCGRecord> call() {
    ...
    try (BufferedReader bufferedReader =
      ...
      CsvToBean<CCGRecord> csvBean =
          new CsvToBeanBuilder<CCGRecord>(bufferedReader).withType(CCGRecord.class).build();
      log.info(
          "Parsed {} CCG csv file to pojos now sorting ... {}ms",
          fileName,
          System.currentTimeMillis() - start);
      ccgRecordList = csvBean.parse().stream().sorted().collect(Collectors.toList());
      log.info(
          "Successfully loaded {} CCG csv. {}ms", fileName, System.currentTimeMillis() - start);
